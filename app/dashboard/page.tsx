...
import { Users, Calendar, ExternalLink, Pencil, List } from "lucide-react";
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 pt-24 pb-10">
                ...
                <div className="mb-8 flex flex-col sm:flex-row sm:items-center gap-4 sm:justify-between">
                    ...
                    <div className="flex flex-row gap-2">
                        {hasPermission(role, "teilnehmerliste:access") && (
                            <Link
                                href="/dashboard/teilnehmer"
                                className="inline-flex items-center gap-2 px-4 py-2.5 border-neutral-900 border text-black text-sm font-semibold rounded-md hover:bg-gray-100 transition-colors self-start sm:self-auto shrink-0"
                            >
                                <List className="w-4 h-4" /> Teilnehmer Listen
                            </Link>
                        )}
                        {hasPermission(role, "dashboard:edit") && (
                            <Link
                                href="/dashboard/edit"
                                className="inline-flex items-center gap-2 px-4 py-2.5 border-neutral-900 border bg-neutral-900 text-white text-sm font-semibold rounded-md hover:bg-neutral-700 transition-colors self-start sm:self-auto shrink-0"
                            >
                                <Pencil className="w-4 h-4" /> Website
                                bearbeiten
                            </Link>
                        )}
                    </div>
