...
export default async function DashboardPage() {
    ...
    const [user, { training, stats }] = await Promise.all([currentUser(), getSiteData()])
            ...
            <header className="bg-white border-b border-neutral-200">
                <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
                    <div className="flex items-center gap-2.5">
                        ...
                        <Link href="/mitglieder" className="text-sm text-neutral-500 hover:text-neutral-900 transition-colors">
                            Mein Bereich
                        </Link>
                    </div>
                ...
                <div className="mb-8 flex items-start justify-between gap-4">
                    <div>
                        ...
                        <p className="text-neutral-500 mt-1">
                            Willkommen zurück, {user?.firstName ?? user?.emailAddresses[0]?.emailAddress}.
                        </p>
                    </div>
                    <Link
                        href="/dashboard/edit"
                    ...
                    <div className="bg-white rounded-xl border border-neutral-200 p-6">
                        ...
                        <div className="space-y-3">
                            {training.sessions.map(s => (
                                <div key={s.id} className="flex items-center justify-between text-sm">
                                    <div className="flex items-center gap-3">
                                        <span className="font-medium text-neutral-900 w-24">{s.day}</span>
                            ...
                            {training.sessions.length === 0 && (
                                <p className="text-sm text-neutral-400">Keine Trainings eingetragen.</p>
                            )}
                        </div>
                    </div>
                </div>
