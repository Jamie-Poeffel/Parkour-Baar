...
import {
    updateAbout, updateStats, updateEmail,
    upsertSocial, deleteSocial,
    updateLocation, upsertSession, deleteSession,
    upsertTechnique, deleteTechnique,
} from "@/app/actions"
import type { SiteData, Session, Social, Technique } from "@/lib/site-data"
import {
    LogOut, ArrowLeft, Plus, Trash2, Save,
    Clock, MapPin, Users, BarChart2, AtSign, Check, BookOpen,
} from "lucide-react"
...
const inputCls = "w-full px-3 py-2.5 text-sm border border-neutral-200 rounded-md focus:outline-none focus:ring-2 focus:ring-neutral-900 focus:border-transparent bg-white"
const textareaCls = `${inputCls} resize-none`

// ── SESSIONS SECTION ──────────────────────────────────────────────────────────
function SessionsSection({ initial, location: initLocation }: { initial: Session[]; location: string }) {
    ...
    const blank = (): Session => ({ id: Date.now().toString(), day: "Dienstag", time: "", level: "", note: "" })
            ...
            <div className="mb-6 pb-6 border-b border-neutral-100">
                <Field label="Trainingsort">
                    <div className="flex gap-2">
                        ...
                        >
                            {savedId === "location" ? <Check className="w-4 h-4" /> : <Save className="w-4 h-4" />}
                        </button>
                    </div>
                </Field>
            </div>
                        ...
                        {editing?.id === s.id ? (
                            ...
                            <div className="flex items-center justify-between px-4 py-3">
                                <div className="flex items-center gap-4 text-sm">
                                    ...
                                    <span className="px-2 py-0.5 bg-neutral-100 text-neutral-600 rounded-full text-xs">{s.level}</span>
                                    <span className="text-neutral-400 text-xs hidden sm:block">{s.note}</span>
                                </div>
                                <div className="flex items-center gap-1">
                                    {savedId === s.id && <Check className="w-4 h-4 text-green-500 mr-1" />}
                                    <button
...
function SessionForm({ session, onChange, onSave, onCancel, isPending }: {
    session: Session
    ...
    return (
        <div className="p-4 space-y-3">
            ...
            <div className="flex justify-end gap-2 pt-1">
                <button onClick={onCancel} className="px-3 py-2 text-sm text-neutral-500 hover:text-neutral-900 transition-colors">
                    Abbrechen
                </button>
                <button
                    onClick={onSave}
...
// ── STATS SECTION ─────────────────────────────────────────────────────────────
function StatsSection({ initial, sessionsCount }: { initial: SiteData["stats"]; sessionsCount: number }) {
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                    {(["members", "years", "coaches"] as const).map(key => (
                        <Field key={key} label={{ members: "Mitglieder", years: "Jahre aktiv", coaches: "Coaches" }[key]}>
                            <input
                                className={inputCls}
                                value={stats[key]}
                    ...
                    <Field label="Trainings/Woche">
                        <div className={`${inputCls} bg-neutral-50 text-neutral-500 cursor-default flex items-center gap-2`}>
                            {sessionsCount}
...
function ContactSection({ initial }: { initial: SiteData["contact"] }) {
    ...
    const blankSocial = (): Social => ({ id: Date.now().toString(), label: "", url: "" })
                        ...
                        {editing?.id === s.id ? (
                            ...
                            <div className="flex items-center justify-between px-4 py-3">
                                <div className="flex items-center gap-3 min-w-0">
                                    {savedId === s.id && <Check className="w-4 h-4 text-green-500 shrink-0" />}
                                    <span className="font-semibold text-sm text-neutral-900 w-24 shrink-0">{s.label}</span>
                                    <span className="text-sm text-neutral-400 truncate">{s.url}</span>
                                </div>
...
function SocialForm({ social, onChange, onSave, onCancel, isPending }: {
    social: Social
...
function TechniquesSection({ initial }: { initial: Technique[] }) {
    ...
    const blank = (): Technique => ({ id: Date.now().toString(), name: "", desc: "" })
                        ...
                        {editing?.id === t.id ? (
                            ...
                            <div className="flex items-center justify-between px-4 py-3">
                                <div className="flex items-center gap-4 min-w-0">
                                    {savedId === t.id && <Check className="w-4 h-4 text-green-500 shrink-0" />}
                                    <span className="font-semibold text-sm text-neutral-900 w-32 shrink-0">{t.name}</span>
                                    <span className="text-sm text-neutral-400 truncate">{t.desc}</span>
                                </div>
                                <div className="flex items-center gap-1 shrink-0 ml-2">
                                    <button
...
function TechniqueForm({ technique, onChange, onSave, onCancel, isPending }: {
    technique: Technique
    ...
    return (
        <div className="min-h-screen bg-neutral-50">
            <header className="bg-white border-b border-neutral-200 sticky top-0 z-10">
                <div className="max-w-4xl mx-auto px-6 h-16 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-semibold text-neutral-900">Website bearbeiten</span>
                    </div>
            ...
            <main className="max-w-4xl mx-auto px-6 py-10 space-y-6">
                <div className="mb-2">
                    <h1 className="text-2xl font-black text-neutral-900 tracking-tight">Website bearbeiten</h1>
                    <p className="text-neutral-500 mt-1 text-sm">Änderungen werden sofort auf der Website aktualisiert.</p>
                </div>

                <SessionsSection initial={data.training.sessions} location={data.training.location} />
                <TechniquesSection initial={data.techniques ?? []} />
                <StatsSection initial={data.stats} sessionsCount={data.training.sessions.length} />
