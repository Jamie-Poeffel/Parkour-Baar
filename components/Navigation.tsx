...
export function Navigation() {
    ...
    const isAdmin = isLoaded && isSignedIn && (roleFromClaims === "admin" || roleFromUser === "admin")
    ...
    const navLinks = [
        ...
        { href: "/kontakt", label: "Kontakt", active: pathname.startsWith("/kontakt") },
    ];

    const linkCls = (active: boolean) => `px-4 py-2 text-sm font-medium rounded-md transition-colors ${
        transparent
    ...
    return (
        <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
            transparent ? 'bg-transparent' : 'bg-white border-b border-neutral-200 shadow-sm'
        }`}>
            <div className="max-w-7xl mx-auto flex items-center justify-between px-6 h-16">
                <Link href="/" className="flex items-center gap-2.5 shrink-0">
                    ...
                    />
                    <span className={`font-bold tracking-tight text-base transition-colors ${transparent ? 'text-white' : 'text-neutral-900'}`}>
                        Parkour Baar
                    </span>
                </Link>
                ...
                <nav className="hidden md:flex items-center gap-1">
                    {navLinks.map(({ href, label, active }) => (
                        <Link key={href} href={href} className={linkCls(active)}>{label}</Link>
                    ))}
